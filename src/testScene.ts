...
export const testScene = (canvas, engine) => {
  ...
  // camera
  const camera = setArcCamera(canvas, scene, null);
  camera.fov = 0.3;
  ...
  var dfltw = canvas.width;
  var dflth = canvas.height;

  engine.onResizeObservable.add(function () {
    camera.orthoTop = 5 * (canvas.height/dflth);
    camera.orthoBottom = -5 * (canvas.height/dflth);
    camera.orthoLeft = -5 * (canvas.width/dfltw);
    camera.orthoRight = 5 * (canvas.width/dfltw);
  });
